...
|---|------|------|-------|-------|----------|--------|---------|
| 01 | recon | — | scout | sonnet | 01-scout.md | DONE | — |
...
check("model spread correct",
      dict(m["by_model"]) == {"sonnet": 2, "opus": 1},
      str(dict(m["by_model"])))
...
m, p = rc.derive(ledger(HEADER + ROUNDS + COST.replace("sonnet 2", "sonnet 7")))
...
m, p = rc.derive(ledger(HEADER.replace("backend-engineer | sonnet |", "backend-engineer | — |") + ROUNDS + COST))
check("unrecorded model flagged", any("records no model" in x for x in p), str(p))

print("escalations")
m, p = rc.derive(ledger(HEADER.replace("backend-engineer | sonnet |", "backend-engineer | sonnet↑ |") + ROUNDS + COST))
check("arrow counted as an escalation", m["escalations"] == 1, str(m["escalations"]))
# Both sonnet rows land under the same key: the escalated dispatch is still a
# sonnet dispatch, and `sonnet↑` must never become a model name of its own.
check("arrow does not corrupt the model name", m["by_model"]["sonnet"] == 2,
      str(dict(m["by_model"])))
m, _ = rc.derive(ledger(HEADER.replace("backend-engineer | sonnet |", "backend-engineer | sonnet^ |") + ROUNDS + COST))
...
print("a model no longer in policy still parses - old ledgers record haiku")
_old = HEADER.replace("| scout | sonnet |", "| scout | haiku |")
_m, _p = rc.derive(ledger(_old + ROUNDS + COST.replace("sonnet 2 · opus 1",
                                                       "sonnet 1 · haiku 1 · opus 1")))
check("haiku counted, not rejected",
      dict(_m["by_model"]) == {"sonnet": 1, "haiku": 1, "opus": 1},
      str(dict(_m["by_model"])))
check("and it is not a problem", _p == [], str(_p))
