...
1. Read `.agentic.json` at the repo root.
2. Read `<agenticRoot>/projects/<project>/project.md` in full.
3. Read `<agenticRoot>/PROTOCOL.md` in full — the work-order, report and
   change-request shapes. **Your report must use the report block defined
   there**, even when you were dispatched directly rather than by the
   orchestrator: downstream lanes are routed from your `contract_notes` and
   `handoff` fields, and prose cannot be routed.
